...
public class WXEventProtocol extends WXBaseProtocol {
	...
	private void dispatchWXEventProtocol() {
		...
		if (Event.equals(WXMessageUtil.EVENT_TYPE_SUBSCRIBE)) {
			...
			// 响应给关注者的文本内容
			respXml = WXProtocolManager
					.respWXTextMsgToXml(requestMap, "谢谢您的关注");
		}
		// 取消关注
		...
		// 成员进入应用的事件
			...
			// 响应给关注者的文本内容
			respXml = WXProtocolManager.respWXTextMsgToXml(requestMap,
					"成员进入应用的事件");
		}
		// 上报地理位置
		...
		// 通讯录变更事件
			...
			// 更新成员事件
					...
					.equals(WXMessageUtil.CHANGE_CONTACT_UPDATE_USER)) {
			...
			// 删除成员事件
					...
					.equals(WXMessageUtil.CHANGE_CONTACT_DELETE_USER)) {
			...
			// 新增部门事件
					...
					.equals(WXMessageUtil.CHANGE_CONTACT_CREATE_PARTY)) {
			...
			// 更新部门事件
					...
					.equals(WXMessageUtil.CHANGE_CONTACT_UPDATE_PARTY)) {
			...
			// 删除部门事件
					...
					.equals(WXMessageUtil.CHANGE_CONTACT_DELETE_PARTY)) {
		...
		// 扫码推事件且弹出“消息接收中”提示框的事件推送
			...
			// 响应给关注者的文本内容
			respXml = WXProtocolManager.respWXTextMsgToXml(requestMap,
					"扫码推事件且弹出“消息接收中”提示框，事件KEY值" + EventKey);
		}
		// 弹出系统拍照发图的事件推送
